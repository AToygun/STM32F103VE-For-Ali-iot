...
void ESP8266_PrintRxData(uint8_t _ch)
{
		...
		UART_SendC(&huart1, &_ch);		/* 将接收到数据打印到调试串口1 */
	#endif
}
...
void ESP8266_PowerOn(void)
{
	...
	*/
	...
	UART_SetBaud(&huart3, 74880);	/* 设置端口波特率为 74880 */
	...
	UART_SetBaud(&huart3,115200);	/* 1表示硬件流控CRS RTS有效;  0表示无需硬件流控 */
...
{
	UART_SendC(&huart3, (uint8_t *)_Cmd);
	UART_SendC(&huart3, (uint8_t *)"\r\n");
}
...
uint16_t ESP8266_RxNew(uint8_t *_pRxBuf, uint8_t *_link_id)
{
	...
	/* +IPD,0,7:ledon 1 */
	while (UART_ReadC(&huart3, &ucData))
	{
		//ESP8266_PrintRxData(ucData);		/* 将接收到数据打印到调试串口1 */
